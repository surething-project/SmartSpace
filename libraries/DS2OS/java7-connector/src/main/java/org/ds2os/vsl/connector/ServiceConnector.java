...
public class ServiceConnector extends AbstractVslModule implements VslParametrizedConnector {
     ...
     *            the password of the keystore of the service.
     */
    public ServiceConnector(final String knowledgeAgent, final String keystorePath,
    ...
    public final void activate() throws CertificateException, NoSuchAlgorithmException, KeyStoreException, IOException {
        synchronized (this) {
            LOG.info("Activate service connector to {} with keystore from {}.", knowledgeAgent,
                    keystorePath);
            ...
            final RestTransportContext context = new RestTransportContext(config,
                    ...
                    mapperFactory);
            jettyClient = new JettyClient(context);
            jettyClient.start();
    ...
    /**
     * Check the connector and throw a proper exception if this module is not activated yet.
     *
