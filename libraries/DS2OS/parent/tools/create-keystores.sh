...
echo "=== creating CA ==="
openssl req -batch -config openssl.cnf -days 3650 -new -newkey rsa:2048 -nodes -sha256 -x509 -keyout ca.key -out ca.crt
chmod 600 ca.key
echo 01 >serial
echo -n "" >index.txt
...
create_cert() {
	...
	openssl req -batch -config openssl.cnf -new -newkey rsa:2048 -nodes -keyout "${commonName}.key" -out "${commonName}.csr"
